...
const PriceCalculator: React.FC<PriceCalculatorProps> = ({ media }) => {
  ...
  useEffect(() => {
    if (media.type === 'game') {
      // Default to HLTB or 4 on load
      setGameDuration(Math.max(4, Math.round(media.hltbTime)));
  ...
  return (
    <div className="w-full max-w-5xl mx-auto">
      <div className="backdrop-blur-xl bg-black/40 border border-white/10 rounded-3xl p-6 md:p-10 shadow-2xl relative overflow-hidden">
            ...
            {media.type === 'game' && (
              <div className="p-3 bg-white/5 rounded-lg border border-white/10">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-xs text-zinc-500 bg-black/40 px-2 py-1 rounded">Min: 4ч | HLTB: {media.hltbTime}ч</span>
                </div>
